...
# USE COMMAND pip install -r requirements.txt the first time around then pip3 freeze > requirements
...
def main():
    ...
    if eventType == "Create":
        ...
        For your response input the following prompt information in the exact format below. Make sure to start your response at the first left curly brace of the event dictionary. Understand that the current time is currently {current_datetime}: 
        ...
        event_description = "\nEvent - Title: " + insert_event_dict["summary"] + "\t\tDescription: " + insert_event_dict["description"] + "\nStart Time: " + insert_event_dict["start"]["dateTime"] + "\t\t\tEnd Time: " + insert_event_dict["end"]["dateTime"] + "\n"
    ...
    # Update Event
        ...
        # User changes
        event_update = str(input('What would you like to change about the event?: '))
        ...
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a helpful assistant. You will update the fields that need updating."},
